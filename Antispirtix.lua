...
local Slider = Tabs.Fly:AddSlider("Slider", {
    Title = "Fly Speed",
    ...
    Callback = function(Value)
        ...
        if not Toggle.Value or not character then
            return -- Если тоггл выключен или персонажа нет, выходим
        end

        -- Устанавливаем скорость полета
        local CFspeed = Value
        ...
        -- Настроим персонажа для полета
        humanoid.PlatformStand = true
        rootPart.Anchored = true

        -- Логика для полета
        if CFspeed > 0 then
            -- Убедимся, что переменная CFloop существует и подключена только один раз
            if not _G.CFloop then
                _G.CFloop = game:GetService("RunService").Heartbeat:Connect(function(deltaTime)
                    if not character or not humanoid then
                        if _G.CFloop then _G.CFloop:Disconnect() end
                        _G.CFloop = nil
                        return
                    end
                    ...
                    local cameraOffset = rootPartCFrame:ToObjectSpace(cameraCFrame).Position
                    ...
                    local objectSpaceVelocity = CFrame.new(cameraPosition, Vector3.new(rootPartPosition.X, cameraPosition.Y, rootPartPosition.Z)):VectorToObjectSpace(moveDirection)
                    rootPart.CFrame = CFrame.new(rootPartPosition) * (cameraCFrame - cameraPosition) * CFrame.new(objectSpaceVelocity)
                end)
            end
        else
            -- Если скорость 0, просто выключаем полет
            if _G.CFloop then
                _G.CFloop:Disconnect()
                _G.CFloop = nil
            end
...
-- Логика для тоггла (включение/выключение полета)
Toggle:OnChanged(function(value)
    ...
    if character then
        ...
        if value then
            -- Включаем полет, когда тоггл активирован
            if humanoid then
                humanoid.PlatformStand = true
            end
            if rootPart then
            ...
            -- Выключаем полет, когда тоггл деактивирован
            if humanoid then
                humanoid.PlatformStand = false
            end
            if rootPart then
...
Tabs.Bots:AddParagraph({
	...
	Tabs.Bots:AddButton({
		Title = "Aimbot",
		Description = "Click to execute aimbot",
		Callback = function()
...
LocalPlayer.CharacterAdded:Connect(function(character)
	...
	Tabs.ScriptHub:AddButton({
		Title = "Gey Love Wt",
		Description = "RPG SPAM NAXUI",
		Callback = function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/Ktulhucc/war-tycoon-script/refs/heads/main/gey%20love%20war%20v2"))()
		end
	})

	Tabs.ScriptHub:AddButton({
		Title = "Fatality yeld",
		Description = "Custom infinite yield | for HvH",
		Callback = function()
...
task.spawn(function()
	...
	Tabs.Credits:AddParagraph({
		Title = "Fatality yeld - Ktulhu",
		Content = "discord - ktulhu.cc",
	})